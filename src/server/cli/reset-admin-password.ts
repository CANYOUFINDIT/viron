...
const result = await db.prepare("UPDATE admin_users SET password_hash = ?, updated_at = ? WHERE username = ?")
...
if (!result.changes) {
  ...
  await db.prepare("DELETE FROM sessions WHERE user_id = (SELECT id FROM admin_users WHERE username = ?)").run(username);
