...
describe("active environment picture-in-picture", () => {
  ...
  it("keeps live workspace surfaces mounted and renders image previews instead of a connection list", () => {
    ...
    expect(desktopDockSmoke).toContain("window.vironActiveEnvironmentDock.drag({ type: 'drag-start'");
    expect(desktopDockSmoke).toContain("window.vironActiveEnvironmentDock.drag({ type: 'drag-move'");
    expect(desktopDockSmoke).toContain("window.vironActiveEnvironmentDock.drag({ type: 'drag-end'");
    expect(desktopDockSmoke).toContain("closeActionDelivered");
    expect(desktopDockSmoke).toContain("window.vironActiveEnvironmentDock.action({ type: 'close-environment'");
    ...
    expect(desktopDockSmoke).toContain("return window.vironActiveEnvironmentDock.action({");
    expect(desktopDockSmoke).toContain("origin: { x: bounds.x, y: bounds.y, width: bounds.width, height: bounds.height }");
    const postCloseSmoke = desktopDockSmoke.slice(desktopDockSmoke.indexOf("const closeStateRemoved"));
    expect(postCloseSmoke.indexOf("const actionDelivered = await actionPromise")).toBeLessThan(postCloseSmoke.indexOf("await updateActiveEnvironmentDockWindow(expandedState)"));
    expect(postCloseSmoke).toContain("panel?.classList.contains('is-expanded')");
    expect(postCloseSmoke).toContain("panel.getBoundingClientRect().height === ${expandedSize.height}");
    ...
    expect(desktopDockSmoke).toContain("const collapseSettleDeadline = Date.now() + 3_000");
    expect(desktopDockSmoke).toContain("collapseEndPanelHeight !== collapsedSize.height");
    expect(desktopDockSmoke).toContain("VIRON_DESKTOP_SMOKE_DIAGNOSTIC collapse-animation");
