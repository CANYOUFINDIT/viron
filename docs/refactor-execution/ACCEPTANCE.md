...
- [x] 只修改技术合同允许的范围；任何例外均有用户批准和 ADR。
- [x] UI/UX 行为符合已批准的 `UIUX-SPEC.md` 2.0.0-final。
- [x] 没有未说明的 Schema、API、权限、错误码或性能限制变化。
- [x] 新逻辑包含行为测试，不只依赖源码字符串断言。
- [x] SQLite 与 MySQL 的真实应用 schema/迁移/CRUD 语义均有证据；skip 不算通过。
- [x] 工作空间隔离、manager/member 和跨空间 404 隐匿测试通过。
- [x] `npm run typecheck` 通过。
- [x] 相关定向测试通过。
- [x] `npm test` 通过，或仅有技术合同明确允许且不影响本阶段的已知非发布问题。
- [x] `npm run build` 通过。
- [x] 需要时完成桌面用户流和启动验证。
- [x] 没有敏感信息、私有数据、`.env`、本地数据和生成的 `release/` 进入提交。
- [x] 改动已提交到 `dev` 并推送 `origin/dev`，未操作 `main`。
- [x] 本阶段包含代码修改时，`npm run package:current-os` 成功；纯文档性阶段标记为“不适用”。
- [x] `STATUS.md` 已记录输入/输出 commit、测试、打包、风险、结论和交接。
...
- [x] 同 deployment 资源锁阻止竞态，成功/失败/timeout 后锁均安全释放。
...
- [x] 维护 Payload、服务操作安全、幂等和故障恢复没有未关闭 P0/P1/P2。
...
- [x] 服务 APM 通过服务级 API 正确聚合，不使用无界 `Promise.all(allHosts)`。
...
- [x] 合并 Gate 中 Phase 2 与 Phase 3 审查项均无未关闭 P0/P1/P2。
...
| `G34-P2-001` | 批量操作 10 分钟总限触发时，已完成目标结果可能未写入 `result_json`，且其他 worker 会在锁释放后继续运行 | 本轮已关闭，不再作为延期项 | 已关闭 | 不适用（未延期） | operation all-settled、锁与部分结果行为测试已通过 |
| `G34-P2-002` | 监控 overview/timeseries 的 MariaDB 规模对账 | 本轮已关闭，不再作为延期项 | 已关闭 | 不适用（未延期） | SQLite/MariaDB 同 fixture 对账已通过 |
...
2026-08-28 Codex 最终关闭说明：`G34-P2-001`、`G34-P2-002` 以及 `G34RR-P1-001`、`G34RR-P2-002`～`G34RR-P2-006` 已全部由行为测试和发布验证关闭，§6～§9 对应条目现已勾选。当前仅 `UX-P3-001` 与 `TLS-DUAL-WRITE` 继续作为已批准、非阻塞延期；合并 Gate 3+4 结论恢复为 `PASSED`。
